...
{
typedef enum {ACT_NONE,ACT_LINEAR,ACT_SIGMOID,ACT_RELU,
...
class None : public Linear
{
public:
	using Linear::function;
	using Linear::function_derivative;
...
inline ActivationFunctionPtr select_activation(Activations::act_t ActVal)
{
	...
	switch (ActVal)
	{
		...
		case Activations::ACT_LINEAR     : { chosen_act = std::make_shared<Activations::Linear>(); break; }
